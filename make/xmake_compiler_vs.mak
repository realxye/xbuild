...
ifeq ($(BUILD_TOOLSET),vs2017)
	...
	VSTOOLSETVER=141
	VSTOOLSET_BINVER=$(XBUILD_TOOLCHAIN_VS2017_MSVC_DEFAULT)
else ifeq ($(BUILD_TOOLSET),vs2019)
	VSROOT=$(XBUILD_TOOLCHAIN_VS2019)
	VSTOOLSETVER=142
	VSTOOLSET_BINVER=$(XBUILD_TOOLCHAIN_VS2019_MSVC_DEFAULT)
else ifeq ($(BUILD_TOOLSET),vs2022)
	VSROOT=$(XBUILD_TOOLCHAIN_VS2022)
	VSTOOLSETVER=143
	VSTOOLSET_BINVER=$(XBUILD_TOOLCHAIN_VS2022_MSVC_DEFAULT)
else
	$(error $(BUILD_TOOLSET) is not supported)
endif

VS_BIN_DIR=$(VSROOT)/VC/Tools/MSVC/$(VSTOOLSET_BINVER)/bin/Host$(XBUILD_HOST_OSARCH)/$(BUILD_ARCH)
VS_INC_DIR=$(VSROOT)/VC/Tools/MSVC/$(VSTOOLSET_BINVER)/include
VS_LIB_DIR=$(VSROOT)/VC/Tools/MSVC/$(VSTOOLSET_BINVER)/lib/$(BUILD_ARCH)
ifeq ($(BUILD_ARCH),$(filter $(BUILD_ARCH),x86 x64))
    # ATL is only valid for x86 and x64
    VS_ATL_INC_DIR=$(VSROOT)/VC/Tools/MSVC/$(VSTOOLSET_BINVER)/atlmfc/include
    VS_ATL_LIB_DIR=$(VSROOT)/VC/Tools/MSVC/$(VSTOOLSET_BINVER)/atlmfc/lib/$(BUILD_ARCH)
endif
