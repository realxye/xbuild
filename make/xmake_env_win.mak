...
$(TARGET_NAME).exe: $(BUILD_PCH) $(BUILD_INTERMEDIATE_TARGETS)
	...
	@if [ ! -z "$(BUILD_VERBOSE)" ] ; then \
		echo '"$(BUILDTOOL_LINK)" $(BUILD_LINK_FLAGS) $(addprefix "$(subst /,\,$(BUILD_INTDIR))\, $(BUILD_INTERMEDIATE_TARGETS)' ; \
	fi
	@"$(BUILDTOOL_LINK)" $(BUILD_LINK_FLAGS) $(addprefix "$(subst /,\,$(BUILD_INTDIR))\, $(BUILD_INTERMEDIATE_TARGETS) || exit 1
