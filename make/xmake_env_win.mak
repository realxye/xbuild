...
ifneq ($(TARGET_PRECOMPILE_HEADER),)
    BUILD_PCH:=$(addsuffix .pch,$(basename $(notdir $(TARGET_PRECOMPILE_HEADER))))
endif
...
# #3: OBJ files
# #4: RES files
BUILD_INTERMEDIATE_TARGETS:=$(BUILD_PCH) $(BUILD_TLBS) $(BUILD_OBJS) $(BUILD_RESES)
...
%.o: $(BUILD_SRCPREFIX)%.c
	...
	@if [ ! -z "$(BUILD_VERBOSE)" ] ; then \
		echo '"$(BUILDTOOL_CC)" $(BUILD_CC_FLAGS) -c $< -Fo"$(subst /,\,$(BUILD_INTDIR))\$@"' ; \
	fi
	@"$(BUILDTOOL_CC)" $(BUILD_CC_FLAGS) -c $< -Fo"$(subst /,\,$(BUILD_INTDIR))\$@" || exit 1
...
%.o: $(BUILD_SRCPREFIX)%.cpp
	...
	@if [ ! -z "$(BUILD_VERBOSE)" ]; then \
		echo '"$(BUILDTOOL_CXX)" $(BUILD_CXX_FLAGS) $(USE_PCHFLAG) -c $< -Fo"$(subst /,\,$(BUILD_INTDIR))\$@"' ; \
	fi
	@"$(BUILDTOOL_CXX)" $(BUILD_CXX_FLAGS) $(USE_PCHFLAG) -c $< -Fo"$(subst /,\,$(BUILD_INTDIR))\$@" || exit 1
...
$(TARGETNAME).pch:
	...
	@if [ "$(TARGETMODE)" = "kernel" ] ; then \
		...
		if [ ! -z "$(BUILD_VERBOSE)" ] ; then \
			echo '"$(BUILDTOOL_CC)" $(BUILD_CXXFLAGS) $(CREATE_PCHFLAG) -c "$(subst /,\,$(BUILD_INTDIR))\$(TARGET_PCHBASENAME).c" -Fo"$(subst /,\,$(BUILD_INTDIR))\$(TARGET_PCHBASENAME).o"' ; \
		fi ; \
		"$(BUILDTOOL_CC)" $(BUILD_CXXFLAGS) $(CREATE_PCHFLAG) -c "$(subst /,\,$(BUILD_INTDIR))\$(TARGET_PCHBASENAME).c" -Fo"$(subst /,\,$(BUILD_INTDIR))\$(TARGET_PCHBASENAME).o" || exit 1 ; \
	else \
		echo '#include "$(TARGET_PCHNAME)"' > $(BUILD_INTDIR)/$(TARGET_PCHBASENAME).cpp ; \
		if [ ! -z "$(BUILD_VERBOSE)" ] ; then \
			echo '"$(BUILDTOOL_CXX)" $(BUILD_CXXFLAGS) $(CREATE_PCHFLAG) -c "$(subst /,\,$(BUILD_INTDIR))\$(TARGET_PCHBASENAME).cpp" -Fo"$(subst /,\,$(BUILD_INTDIR))\$(TARGET_PCHBASENAME).o"' ; \
		fi ; \
		"$(BUILDTOOL_CXX)" $(BUILD_CXXFLAGS) $(CREATE_PCHFLAG) -c "$(subst /,\,$(BUILD_INTDIR))\$(TARGET_PCHBASENAME).cpp" -Fo"$(subst /,\,$(BUILD_INTDIR))\$(TARGET_PCHBASENAME).o" || exit 1 ; \
	fi
...
$(TARGET_NAME).lib: $(BUILD_INTERMEDIATE_TARGETS)
	@if [ ! -d $(BUILD_OUTDIR) ] ; then \
		mkdir -p $(BUILD_OUTDIR) ; \
	fi
	@if [ ! -z "$(BUILD_VERBOSE)" ] ; then \
		echo '"$(BUILDTOOL_LIB)" $(BUILD_LIB_FLAGS) -OUT:"$(subst /,\,$(BUILD_OUTDIR))\$@" $(addprefix "$(subst /,\,$(BUILD_INTDIR))\, $(addsuffix ",$(BUILD_INTERMEDIATE_TARGETS)))' ; \
	fi
	@"$(BUILDTOOL_LIB)" $(BUILD_LIB_FLAGS) -OUT:"$(subst /,\,$(BUILD_OUTDIR))\$@" $(addprefix "$(subst /,\,$(BUILD_INTDIR))\, $(addsuffix ",$(BUILD_INTERMEDIATE_TARGETS))) || exit 1

# Rule for link DLL
$(TARGET_NAME).dll: $(BUILD_INTERMEDIATE_TARGETS)
	@if [ ! -d $(BUILD_OUTDIR) ] ; then \
		mkdir -p $(BUILD_OUTDIR) ; \
	fi
	@if [ ! -z "$(BUILD_VERBOSE)" ] ; then \
		echo '"$(BUILDTOOL_LINK)" $(BUILD_LINK_FLAGS) /out "$(subst /,\,$(BUILD_OUTDIR))" $<' ; \
	fi
	@"$(BUILDTOOL_LINK)" $(BUILD_LINK_FLAGS) /out "$(subst /,\,$(BUILD_OUTDIR))" $< || exit 1

# Rule for link EXE
$(TARGET_NAME).exe: $(BUILD_INTERMEDIATE_TARGETS)
	@if [ ! -d $(BUILD_OUTDIR) ] ; then \
		mkdir -p $(BUILD_OUTDIR) ; \
	fi
	@if [ ! -z "$(BUILD_VERBOSE)" ] ; then \
...
# Rule for link SYS
$(TARGET_NAME).sys: $(BUILD_INTERMEDIATE_TARGETS)
	@if [ ! -d $(BUILD_OUTDIR) ] ; then \
		mkdir -p $(BUILD_OUTDIR) ; \
	fi
	@if [ ! -z "$(BUILD_VERBOSE)" ] ; then \
